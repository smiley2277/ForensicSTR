    ...
    for i in range(1,ws_flanking.max_row):
        ...
        for reads in samfile.fetch(chrom, start, stop):
            ...
            if 'Amelogenin' in Locus:
                ...
                content = [Locus, Length," ",Sequence,stop,start]
                #writer.writerow(content)
                continue
        merge_list = []
        for indexa in range(0,(len(seq_list)-1)):
            merge_list.append(seq_list[indexa])
            for indexb in range(0,(len(mutate_seq_list)-1)):
                if indexb <= len(mutate_seq_list)-1 :
                    if mutate_seq_list[indexb] == seq_list[indexa]:
                        merge_list.append(mutate_seq_list[indexb])
                        mutate_seq_list.pop(indexb)
                        indexb-=1
                    elif mutate_seq_list[indexb] != seq_list[indexa]:
                        continue
                else:
                    continue
        print("locus:", Locus, len(merge_list))
        ...
        for index,element in enumerate(reads_list):
            if (reads_list[index][1] < 10 and len(merge_list)> 10 ) :
                continue
            elif ( reads_list[index][1] < 10 and len(merge_list) <=10 ):
                if (Locus == locus_list[i+1+z] and Locus ==locus_list[i+2+z]) :
                ...
                writer.writerow(content)
                print(content,reads_list[index][1],"locus_reads:",locus_reads)
